...
/*{
    ...
    public Transform AICatchPoint;
    public Transform shootingPoint;
    public Transform AIShootingPoint;
    ...
    void Update()
    {
        ...
        {
            // catch the ball
            gameObject.transform.position = new Vector3(AICatchPoint.position.x, AICatchPoint.position.y, AICatchPoint.position.z);
            sc.enabled = false; // collider set to false
            isBallGrounded = false;
            isBallToAI = true;
        }
        ...
        {
            gameObject.transform.position = new Vector3(AIShootingPoint.position.x, AIShootingPoint.position.y, AIShootingPoint.position.z);
            rb.velocity = (AIShootingPoint.transform.forward * throwSpeed);  // throw to forward direction
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        else if (other.gameObject.CompareTag("AILegSensor") && catchValue != 1)
        {
            //gameObject.transform.position = new Vector3(point.position.x, point.position.y, point.position.z);
...
*/
{
    ...
    public static bool isBallGrounded;
    public static bool isBallToAI;
    public static bool isBallToPlayer;
